 ...
 *  <tr> <td>1.0		<td>2004-6-8	<td>	<td>Create this file</tr>
 *  <tr> <td>1.1		<td>2005-9-29	<td>	<td>进行了梳理</tr>
 * </table>
 * 
 */
...
namespace sp
{
	...
	*/
	 ...
	 * @see		
	 */
	int split_str(const char * str, str_arr & subs_array, const char spliter[] = ",;:", bool trim = true, bool ignore_blank = false, bool supp_quote = false, bool trim_quote = true, bool supp_quanjiao = false, char * longer_subs = 0, int longer_len = 0);
	int split_str(const wchar_t * str, wstr_arr & subs_array, const wchar_t spliter[] = L",;", bool trim = true, bool ignore_blank = false, bool supp_quote = false, bool trim_quote = true, char * longer_subs = 0, int longer_len = 0);
	int split_str(const char * str, wstr_arr & subs_array, const wchar_t spliter[] = L",;", bool trim = true, bool ignore_blank = false, bool supp_quote = false, bool trim_quote = true, char * longer_subs = 0, int longer_len = 0);
	int split_str(const wchar_t * str, str_arr & subs_array, const char spliter[] = ",;:", bool trim = true, bool ignore_blank = false, bool supp_quote = false, bool trim_quote = true, char * longer_subs = 0, int longer_len = 0);
	 ...
	 * @see		
	 */
	int str_to_attr_vals(const char* str, str_map & attrs, bool trim = false, bool supp_quote = false, const char * splt = " ;,", const char * token = "=", char * longer_subs = 0, int longer_len = 0 );
	int str_to_attr_vals(const char* str, str_pair_arr & attrs, bool trim = false, bool supp_quote = false, const char * splt = " ;,", const char * token = "=", char * longer_subs = 0, int longer_len = 0);
...
namespace sp
{
	...
	*/
	 ...
	 * @param	bool attr_hide = false	- [in]  是否获取隐藏的文件
	...
	int get_file_list(const char* dir, LIST & flist, const char* filter = 0, bool recursive = true, FF_PROC fproc = ff_proc_default(), bool attr_hide = false);
	template<class LIST>
	int get_dir_list (const char* dir, LIST & dlist, const char* filter = 0, bool recursive = true, bool attr_hide = false);
	template<class LIST, class FF_PROC>
	int get_dir_list (const char* dir, LIST & dlist, const char* filter = 0, bool recursive = true, FF_PROC fproc = ff_proc_default(), bool attr_hide = false);
